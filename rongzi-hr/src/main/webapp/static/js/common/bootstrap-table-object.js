                ...
                $('#' + tableId).bootstrapTable({
                    ...
                    rowStyle: function (row, index) {
                        if ((row.ProductionDate) != null) {
                            var day = new Date(row.ProductionDate).getDay();
                            if (day == 0 || day == 6) {
                                var style = {};
                                style = {css: {'background-color': '#8397AD'}};
                                return style;
                            }
                        }
                        if ((row.createdate) != null) {
                            var day = new Date(row.createdate).getDay();
                            if (day == 0 || day == 6) {
                                var style = {};
                                style = {css: {'background-color': '#8397AD'}};
                                return style;
...
function toolTipFormatter(value, row, index) {
    var shortString = "";
    if (value.length > 100) {
        shortString = value.substring(0, 100) + '...';
    } else {
        shortString = value;
    }
    var fullValue = "<div style='text-align:left; font-size:16px;'>" + value + "</div>";
    return [
        '<span data-toggle="tooltip" data-placement="auto right" data-html="true" title="' + fullValue + '">' + shortString + '</span>',
    ].join('');
}
